...
def op(x:tuple[int,int,int,int], y:tuple[int,int,int,int]) -> tuple[int,int,int,int]:
    v1x, c1x, v2x, c2x = x
    v1y, c1y, v2y, c2y = y
    if v1x == v1y:
...
e = (-1,0,-2,0)
X:list[tuple[int,int,int,int]] = [(a,1,-1,0) for a in A]
st = SegTree(op, e, X)

for _ in range(Q):
    I = input().split()
    q, p, x = int(I[0]), int(I[1]), int(I[2])
    if q == 1:
        st.set(p-1, (x,1,-1,0))
    else:
